...
export const getCredentialConfigsBasedOnFormatPref = async (
    ...
    Object.entries(credentials)
        .filter(([_, config]) => !config.format || (config.format.toLowerCase() in vcFormatPreferences.map(pref => pref.toLowerCase())))
        .forEach(
            ([key, config]) => {
...
export const getOrCreatePrimaryIdentifier = async (args: GetOrCreatePrimaryIdentifierArgs): Promise<IIdentifier> => {
    ...
    const identifier: IIdentifier = !identifiers || identifiers.length == 0 ? await createIdentifier({
        context,
        opts
    }) : identifiers[0]
