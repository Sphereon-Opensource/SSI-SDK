...
export class OID4VCIHolder implements IAgentPlugin {
  ...
  private async oid4vciHolderGetCredentialOfferData(args: InitiateOID4VCIArgs, context: RequiredContext): Promise<InitiationData> {
    ...
    if (
      !requestData?.uri ||
      !(requestData?.uri.startsWith(RequestType.OPENID_INITIATE_ISSUANCE) || requestData?.uri.startsWith(RequestType.OPENID_CREDENTIAL_OFFER) || requestData?.uri.startsWith(RequestType.HTTPS))
    ) {
