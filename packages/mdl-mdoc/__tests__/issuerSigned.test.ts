...
import LocalDateTimeKMP = mdoc.com.sphereon.kmp.LocalDateTimeKMP
...
describe('Issuer Auth', (): void => {
  ...
  it('should decode and encode ISO Test Vector', () => {
    const coseSign = mdoc.com.sphereon.crypto.cose.CoseSign1Cbor.Static.cborDecode<any>(decodeFrom(iso18013_5_IssuerAuthTestVector, Encoding.HEX))
    ...
    expect(coseSign.toBeSignedJson(null, mdoc.com.sphereon.crypto.generic.SignatureAlgorithm.ECDSA_SHA256).base64UrlValue).toEqual(
  ...
  it('should verify IETF Test Vector', async () => {
    ...
    const issuerAuth = mdoc.com.sphereon.crypto.cose.CoseSign1Cbor.Static.cborDecode(decodeFrom(ietfTestVector, Encoding.HEX)) as CoseSign1Cbor<any>
  ...
  it('should verify Issuer Signed Test Vector', async () => {
    ...
    const issuerSigned = mdoc.com.sphereon.mdoc.data.device.IssuerSignedCbor.Static.cborDecode(decodeFrom(funkeTestVector, Encoding.HEX))
  ...
  it('should verify Funke Issuer Signed Test Vector of 12-08-2024', async () => {
    const issuerSigned = mdoc.com.sphereon.mdoc.data.device.IssuerSignedCbor.Static.cborDecode(
  ...
  it.skip('should show a full flow from issuer to holder to RP', async () => {
    ...
    // RP side
    const rpDeviceResponse = mdoc.com.sphereon.mdoc.data.device.DeviceResponseCbor.Static.cborDecode(
      decodeFrom(authResponse.vp_token, Encoding.BASE64URL),
    )
