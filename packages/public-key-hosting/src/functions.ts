...
export const jwksURIFromIdentifier = (args: { identifier: IIdentifier, basePath?: string, baseURL?: string }) => {
    const {identifier, baseURL} = args
    let basePath = args.basePath ?? JWKS_HOSTING_DID_KEYS_PATH
    const did = encodeBase64url(identifier.did)
