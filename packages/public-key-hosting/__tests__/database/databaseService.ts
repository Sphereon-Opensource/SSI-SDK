...
export const getDbConnection = async (dbName: string): Promise<DataSource> => {
  ...
  if (sqliteConfig.migrationsRun) {
    ...
    logger.debug(`Running ${dataSource.migrations.length} migration(s) from code if needed...`)
    await dataSource.runMigrations()
    logger.debug(`${dataSource.migrations.length} migration(s) from code were inspected and applied`)
  }
  return dataSource
}
