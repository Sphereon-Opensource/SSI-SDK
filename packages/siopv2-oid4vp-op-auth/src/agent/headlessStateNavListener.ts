...
const logger = Loggers.DEFAULT.options('sphereon:siopv2:op-auth', { defaultLogLevel: LogLevel.DEBUG, methods: [LogMethod.CONSOLE] }).get(
  'sphereon:siopv2:op-auth',
)

export const SIOPV2CallbackStateListener = (
  callbacks?: Map<Siopv2MachineStates, (machine: Siopv2MachineInterpreter, state: Siopv2MachineState, opts?: any) => Promise<void>>,
) => {
  return async (siopv2Machine: Siopv2MachineInterpreter, state: Siopv2MachineState): Promise<void> => {
    if (state._event.type === 'internal') {
      logger.debug('siopv2CallbackStateListener: internal event')
