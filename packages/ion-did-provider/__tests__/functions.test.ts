...
import { generatePrivateKeyHex, tmpMemoryKey, toIonPrivateKeyJwk } from '../src/functions'
...
describe('functions: ionKeys', () => {
  ...
  it('temp recovery Memory Key should be deterministic and have a commitment ', async () => {
    const tmpKey = await tmpMemoryKey(KeyType.Secp256k1, PRIVATE_RECOVERY_KEY_HEX, 'test-recovery-kid', 'test-recovery-kms', {
    ...
    expect(tmpKey).toMatchObject<ManagedKeyInfo>({
      ...
      "publicKeyHex": "04d530f20a7b3aa14a1dd4ca0aa67fc36138b6547bc91f454bda8d37f9021e0f5c24eeb53256a81d1b26ac342b00b0e7346b38f25a47c3cf233a0601714ae63b8b",
      "type": "Secp256k1"
    })
  })
  it('temp update Memory Key should be deterministic and have a commitment ', async () => {
    const tmpKey = await tmpMemoryKey(KeyType.Secp256k1, PRIVATE_UPDATE_KEY_HEX, 'test-update-kid', 'test-update-kms', {
    ...
    expect(tmpKey).toMatchObject<ManagedKeyInfo>({
      ...
      "publicKeyHex": "04826d51d96e9accdef1b13d9acfab61a15d5b5a6b0c1050acb68d58070a3baa0402dae4c63691a6d094537930aef1fa9116af81a6796015edb67013a2792e0b09",
      "type": "Secp256k1"
    })
  })
})
