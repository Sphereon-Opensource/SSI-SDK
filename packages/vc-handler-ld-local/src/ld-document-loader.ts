...
export class LdDocumentLoader {
    ...
    constructor(options: {
        ldContextLoader: LdContextLoader; ldSuiteLoader: LdSuiteLoader, documentLoader?: {
    ...
    getLoader(
        ...
        return extendContextLoader(async (url: string) => {
            ...
            if (url.toLowerCase().startsWith('did:')) {
                const resolutionResult = await getAgentResolver(context, {localResolution: this.localResolution, resolverResolution: this.resolverResolution, uniresolverResolution: this.uniresolverResolution}).resolve(url)
