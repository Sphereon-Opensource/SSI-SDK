...
if (configJSON && Array.isArray(configJSON.configs)) {
    ...
    for (const config of configs.configs) {
        ...
        const result = wallets.map(wallet => {
            ...
            return Promise.all(importedKeys).then(
                walletsKeys => {
                    console.log('=============KEY===========')
                    console.log(JSON.stringify(walletsKeys))
                    console.log('=============KEY===========')

                    const kmsSigners = walletsKeys.filter(key => key !== undefined).map(key => new EthersKMSSignerBuilder().withContext(context).withKeyRef(key as ManagedKeyInfo).build())
                    let signers: Signer[]
