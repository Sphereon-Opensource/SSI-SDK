...
export class DidAuthSiopOpAuthenticator implements IAgentPlugin {
  ...
  private async getDidSiopAuthenticationRequestFromRP(args: IGetDidSiopAuthenticationRequestFromRpArgs, context: IRequiredContext): Promise<ParsedAuthenticationRequestURI> {
    return fetch(`${args.redirectUrl}?stateId=${args.stateId}`)
      .then(async (response: Response) =>
        (response.status >= 400)
            ? Promise.reject(new Error(await response.text()))
            : this.op.parseAuthenticationRequestURI(await response.text())
      )
      .catch((error: unknown) => Promise.reject(error));
  }

  /** {@inheritDoc IDidAuthSiopOpAuthenticator.getDidSiopAuthenticationRequestDetails} */
  private async getDidSiopAuthenticationRequestDetails(args: getDidSiopAuthenticationRequestDetailsArgs, context: IRequiredContext): Promise<IAuthRequestDetails> {
    const presentationDefs = args.verifiedAuthenticationRequest.presentationDefinitions;
    const verifiablePresentations = (presentationDefs && presentationDefs.length > 0)
        ? await this.matchPresentationDefinitions(presentationDefs, args.verifiableCredentials)
        : [];
  ...
  private async matchPresentationDefinitions(presentationDefs: PresentationDefinitionWithLocation[], verifiableCredentials: VerifiableCredential[]): Promise<IMatchedPresentationDefinition[]> {
  ...
  private async verifyDidSiopAuthenticationRequestURI(args: IVerifyDidSiopAuthenticationRequestUriArgs, context: IRequiredContext): Promise<VerifiedAuthenticationRequestWithJWT> {
  ...
  private async sendDidSiopAuthenticationResponse(args: ISendDidSiopAuthenticationResponseArgs, context: IRequiredContext): Promise<Response> {
    return this.op.createAuthenticationResponse(
