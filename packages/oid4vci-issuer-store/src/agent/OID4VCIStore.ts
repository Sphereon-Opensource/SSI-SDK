...
export class OID4VCIStore implements IAgentPlugin {
    ...
    private async oid4vciStoreGetIssuerOpts({
                                                correlationId,
                                                storeId,
                                                namespace
                                            }: Ioid4vciStoreGetArgs): Promise<IIssuerOptions | undefined> {
        return (await this.store({stores: this._optionStores, storeId}).get(this.prefix({
            namespace,
            correlationId
        }))) ?? this.defaultOpts
    }

    private async oid4vciStoreHasIssuerOpts({
                                                correlationId,
                                                storeId,
                                                namespace
                                            }: Ioid4vciStoreExistsArgs): Promise<boolean> {
    ...
    private async oid4vciStoreRemoveIssuerOpts({
                                                   storeId,
                                                   correlationId,
                                                   namespace
                                               }: Ioid4vciStoreRemoveArgs): Promise<boolean> {
    ...
    private async oid4vciStoreGetMetadata({
                                              ...
                                              storeId,
                                              namespace
                                          }: Ioid4vciStoreGetArgs): Promise<CredentialIssuerMetadata | undefined> {
        return this.store({stores: this._metadataStores, storeId}).get(this.prefix({namespace, correlationId}))
    }

    private async oid4vciStoreHasMetadata({
                                              correlationId,
                                              storeId,
                                              namespace
                                          }: Ioid4vciStoreExistsArgs): Promise<boolean> {
    ...
    private store<T extends ValueStoreType>(args: {
